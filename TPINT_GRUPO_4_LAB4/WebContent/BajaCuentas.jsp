...
<thead>
<tr><th>Nro. de cuenta</th><th>Tipo</th><th>CBU</th><th>Saldo</th><th>Eliminar</th></tr>
</thead>
<tbody>
<%
...
%>
<tr><form action="servletCuenta?NroCuentaBaja=<%=user.getNroCuenta() %>" method="get">
<td><%=user.getNroCuenta()%> <input type="hidden" name="NroCuentaBaja" value="<%=user.getNroCuenta()%>"></td>
<td><%=user.getTipoCuenta()%></td>
<td><%=user.getCBU()%></td>
<td><%=user.getSaldo() %></td>
    <td><input type="submit" class="button-9" name="btnEliminarCuenta" value="Eliminar"></td>
    </form></tr> 
<%}%>
</tbody>
</table>
